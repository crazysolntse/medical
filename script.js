async function askAI() {
  ...
  try {
    const res = await fetch("https://openrouter.ai/api/v1/chat/completions ", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "Ты знаток ресторанов и кофеен. Посоветуй 3 классных места, где поесть. Покажи название, далее предложение, которое описывает преимущество и особенности места. Далее ценовая категория (₽,₽₽,₽₽₽). Далее Номер телефона. Далее гео (метро и название улицы)"
          },
          {
            role: "user",
            content: question
    ...
    responseDiv.textContent = marked.parse(answer);
  } catch (err) {
    console.error(err);

    ...
    const res = await fetch("https://openrouter.ai/api/v1/chat/completions ", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "Ты знаток ресторанов и кофеен в Москве. Посоветуй топовое место, где поесть. Сначала название, далее идут два предложения, которые описывают преимущества и особенности места. Далее ценовая категория (₽,₽₽,₽₽₽). Номер телефона. Далее гео (метро и название улицы). Далее ссылка на сайт. ОТформатируй ответ чтобы можно было вставить в середину Html страницы с разметкой"
          },
          {
            role: "user",
            content: question
